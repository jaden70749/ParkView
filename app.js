...
- zones는 빈 배열로 둔다. 차로를 별도 요소로 그리지는 않지만, 주차열 사이의 빈 바닥 공간으로 위치와 폭을 반드시 보존한다. 보행로·완충 구역·방·기둥은 그리지 않는다.
...
- 사진 속 차량 진행 화살표와 사선 빗금 완충 구역은 주차칸이나 element로 그리지 않는다. 하지만 화살표가 놓인 넓은 빈 영역은 명백한 중앙 차로이므로 절대 삭제하거나 주차열로 채우지 않는다.
...
- 위·아래 또는 좌·우 주차열이 마주 보고 그 사이에 화살표가 있으면, 두 열의 가까운 모서리 사이를 중앙 차로로 비워 둔다. 사진에서 차로 폭이 주차칸 긴 변과 비슷하거나 더 넓으면, 탑뷰 좌표에서도 두 열의 안쪽 경계 사이 간격을 최소 주차칸 깊이만큼 남긴다.
...
10. statuses와 kinds는 사진에서 확인되는 경우에만 수정한다. 색상 배치도에서는 초록=available, 회색=occupied, 파랑=disabled, 분홍=pregnant를 보존하고 장식이나 추측 구조물을 추가하지 않는다.
11. detectedSlotCount와 모든 rows의 count 합계가 반드시 같아야 한다.

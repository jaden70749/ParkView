...
고정 CCTV에서 `바로 분석하기`를 누르면 도면의 모든 주차칸이 CCTV 주차장 영역 안에 자동 배치됩니다. YOLO가 인식한 객체 중 중심점이 개별 주차칸 안에 들어온 객체만 해당 칸의 `주차 중` 판정과 화면 표시에 사용하며, 주차장 밖의 사람·의자 같은 객체는 제외합니다. `CCTV 좌표 확인·보정`은 자동 좌표가 어긋났을 때만 사용합니다.
